...
const { data: data, pending: projectPending, error: projectError, refresh: refreshProject } = await useProject(projectId)
const { data: allProjects, pending: allProjectsPending, error: allProjectsError, refresh: refreshAllProjects } = await useProjects()

// Computed properties with null checks
const project = computed(() => data.value || null)
const allProjectsData = computed(() => allProjects.value || [])

// Check if any data is still loading
const isLoading = computed(() => projectPending.value || allProjectsPending.value)

// Check if any critical errors occurred
const hasError = computed(() => projectError.value || allProjectsError.value)

// Error messages
const errorMessage = computed(() => {
  if (projectError.value) return 'Failed to load project details'
  if (allProjectsError.value) return 'Failed to load related projects'
  return 'Something went wrong while loading the project'
})
...
// Meta tags
useHead({
  title: project.value ? project.value.name : 'Project Not Found',
  meta: [
    {
      name: 'description',
...
const selectedImage = computed(() => {
  if (project.value && project.value.images && project.value.images.length > 0) {
    return project.value.images[selectedImageIndex.value]
  }
  return null
})
    ...
    <div v-if="isLoading" class="min-h-screen bg-gray-50">
      ...
      <section class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <LoadingSkeleton type="project-detail" />
        </div>
      </section>
      ...
      <section class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="h-8 bg-gray-200 rounded-lg mx-auto max-w-xs mb-12 animate-pulse"></div>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div v-for="i in 3" :key="i" class="bg-white rounded-2xl shadow-lg p-6 space-y-4">
    ...
    <div v-else-if="project">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-2 space-y-12">
                ...
                <div class="space-y-6">
                  ...
                  <div v-if="project.images.length > 1" class="grid grid-cols-3 md:grid-cols-4 gap-4">
                    <button v-for="(image, index) in project.images" :key="image.id" @click="selectedImageIndex = index"
                      ...
                      <img :src="image.image_url" :alt="image.alt_text || 'Project thumbnail'" class="w-full h-24 object-cover">
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="bg-white rounded-2xl shadow-lg p-8">
                  <div class="rich-text" v-html="project.description || 'No description available'">
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="space-y-8">
              ...
              <!-- techs Used -->
              <div v-if="project.techs && project.techs.length > 0" class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-4">
                  <div v-for="tech in project.techs" :key="tech.id" class="border-l-4 border-blue-500 pl-4">
                    <h4 class="font-semibold text-gray-900">{{ tech.name || 'Unknown Technology' }}</h4>
                    <p class="text-sm text-gray-600 mt-1">{{ tech.description || 'No description available' }}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div v-if="relatedProjects.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <ProjectCard v-for="relatedProject in relatedProjects" :key="relatedProject.id" :project="relatedProject" />
          </div>
          <div v-else class="text-center py-12">
            <p class="text-gray-600 text-lg">No related projects available.</p>
          </div>
