...
const categories = computed(() => {
  ...
  return [...new Set(allSkills.map(skill => skill.category_name.toLowerCase()))]
})
...
const filteredSkills = computed(() => {
  ...
  return allSkills.filter(skill => skill.category_name.toLowerCase() === selectedcategory_name.value)
})

// Methods
const getcategory_nameSkills = (category_name) => {
  return allSkills.filter(skill => skill.category_name.toLowerCase() === category_name)
}
    ...
    <section class="py-20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-wrap justify-center gap-4 mb-12">
          ...
          <button v-for="category_name in categories" :key="category_name" @click="selectedcategory_name = category_name" :class="[
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div v-for="category_name in categories" :key="category_name" class="text-center">
              ...
              <p class="text-gray-600 text-sm">
                {{ getcategory_nameSkills(category_name).length }} {{ getcategory_nameSkills(category_name).length === 1 ? 'skill' :
                  'skills' }}
              </p>
            </div>
          </div>
